...
class StaggEKGPro_Config:
    ...
    async def turn_on_smart(self, temp_celsius: float = None):
        ...
        This uses a "schedule hack" mechanism:
        1. Backs up your current schedule (e.g., Daily at 7:00 AM)
        2. Sets schedule to trigger 1 minute in the future (forces kettle to start)
        3. Waits 65 seconds for the kettle to activate and start heating
        4. Restores your original schedule

        IMPORTANT: This requires the kettle's clock to be set correctly!
        If the clock time is wrong, the schedule trigger won't work properly.

        Args:
            temp_celsius: Target temperature in Celsius (uses current target if not specified)

        Raises:
            RuntimeError: If not connected or if state data is unavailable
        """
        if not self.client or not self.client.is_connected:
            raise RuntimeError("Not connected to kettle")
        ...
        logger.info(
            "Backup: Saved existing schedule (%s @ %s)",
            backup_sched.get('schedule_mode'),
            backup_sched.get('schedule_time')
        )

        # --- STEP 2: TRIGGER ---
        # Calculate current time from kettle's clock (bytes 11 and 10)
        current_h = self._state_data[11]
        current_m = self._state_data[10]

        # Set trigger time to 1 minute in the future
        ...
        # Use provided temp or current target temp
        target_temp = temp_celsius if temp_celsius is not None else (self._state_data[4] / 2.0)

        # Clamp temperature to valid range
        target_temp = max(40, min(100, target_temp))

        logger.info(
            "Triggering: Scheduling ONCE at %02d:%02d with target %.1f°C",
            trigger_h, trigger_m, target_temp
        )

        # Set temporary ONCE schedule to trigger kettle
        ...
        # Wait for kettle to recognize the schedule trigger and start heating
        # This needs to be long enough for the kettle to lock in the heating cycle
        ...
        if backup_sched.get('schedule_enabled'):
            ...
            logger.info(
                "Restoring: Setting schedule back to %s at %s",
                restore_mode.name,
                backup_sched['schedule_time']
            )
            ...
            logger.info("Restoring: Disabling schedule (was originally off)")
            await self.set_schedule(mode=ScheduleMode.OFF)

        logger.info("Turn on sequence completed")
